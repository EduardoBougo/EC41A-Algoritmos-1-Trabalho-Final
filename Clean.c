...
void menu(void)
{
    ...
    do
    {
        ...
        case 7:
            cabecalho("CONSULTAR VALORES PENDENTES/CONCLU�DOS");
            valoresPendentes();
            enterParaContinuar();
            break;
        ...
        case 11:
            cabecalho("CARROS POR ORDEM ALFAB�TICA");
...
void cadastrarCliente(void)
{
    ...
    do // Loop. Recebe o telefone e checa se � V�lido
    {
        ...
        if (!validar)
        {
            printf(">>>Telefone Invalido: H� mais ou menos caracteres que o desejado.<<<\n");
        }
    } while (!validar);
...
void cadastrarCarro(void)
{
    ...
    do // Loop. Recebe o ano e checa se � V�lido
    {
        ...
        if (!validar)
        {
            printf(">>>Ano n�o pode ser posterior ao atual<<<");
        }
...
void buscarClientePeloNome(char nome[50])
{
    ...
    if (encontrado == 1)
    {
        ...
        printf("N�o foi encontrado nenhum cliente com nome %s", nome);
    }
}
        ...
        for (j = 0; j < g_qtdServicos; j++)
        {
            if (strcmp(carroAtual.placa, g_servicos[j].carro.placa) == 0)
            {
                ...
                if (g_servicos[j].pago == 'N')
                {
                    ...
                    {
                    case 1:
                        printf("Servi�o de Revis�o b�sica - pago\n");
                        break;
                    case 2:
                        printf("Servi�o de Troca de �leo - pago\n");
                        break;
                    case 3:
                        printf("Servi�o de Alinhamento e balanceamento - pago\n");
                        break;
                    case 4:
                        printf("Servi�o de Higieniza��o - pago\n");
                        break;
                    }
                }
            }
...
void promocaoAniversario(int dia, int mes) {
    ...
    for (i = 0; i < g_qtdClientes; i++)
    {
        ...
        {
            printf("O cliente %s foi selecionado para a promo��o de anivers�rio do dia %d ao dia %d no m�s de %s!\n", g_clientes[i].nome, dia, (dia + 7), meses[mes - 1]);
            encontrado = 1;
        }
    }
    ...
    if (!encontrado)
    {
        printf("N�o h� nenhum cliente que faz anivers�rio do dia %d ao dia %d no m�s de %s", dia, (dia + 7), meses[mes - 1]);
    }
